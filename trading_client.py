...
def main():
    ...
    trading_client = TradingClient(os.environ.get("API_KEY"), os.environ.get("API_SECRET"))
    data_client = StockHistoricalDataClient(os.environ.get("API_KEY"), os.environ.get("API_SECRET"))
        ...
        if status == "open":
            ...
            if not ndaq_tickers:
                ndaq_tickers = get_ndaq_tickers(
                    mongo_url, os.environ.get("FINANCIAL_PREP_API_KEY")
                )  # Fetch tickers using the helper function
            ...
            for ticker in ndaq_tickers:
                ...
                try:
                    ...
                    portfolio_collection.update_one(
                        {"name": "ndaq_percentage"}, {"$set": {"portfolio_value": (qqq_latest - 503.17) / 503.17}}
                    )
                    portfolio_collection.update_one(
                        {"name": "spy_percentage"}, {"$set": {"portfolio_value": (spy_latest - 590.50) / 590.50}}
                    )
                    ...
                    if (
                        decision == "buy"
                        and float(account.cash) > 15000
                        and (((quantity + portfolio_qty) * current_price) / portfolio_value) < 0.1
                    ):
                        ...
                        order = place_order(
                            trading_client, ticker, OrderSide.SELL, qty=quantity, mongo_url=mongo_url
            ...
            while buy_heap and float(account.cash) > 15000:
                try:
                    ...
                    order = place_order(
                        trading_client, ticker, OrderSide.BUY, qty=quantity, mongo_url=mongo_url
                    )  # Place order using helper
