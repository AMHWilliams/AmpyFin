...
def adaptive_momentum_filter_strategy(
    ...
    sentiment = 50 + (
        momentum_signal * 100 * trend_strength.iloc[-1] + trend_signal * 100 * (1 - trend_strength.iloc[-1])
    )
...
def information_flow_strategy(
    ...
    info_flow = mutual_information(price_changes[-window:].fillna(0), volume_changes[-window:].fillna(0))
