...
def bollinger_bands_strategy(
    ticker, current_price, historical_data, account_cash, portfolio_qty, total_portfolio_value
):
    """
...
def heikin_ashi_strategy(ticker, current_price, historical_data, account_cash, portfolio_qty, total_portfolio_value):
    ...
    ha_close = (
        historical_data["open"] + historical_data["high"] + historical_data["low"] + historical_data["close"]
    ) / 4
...
def volume_weighted_macd_strategy(
    ...
    vwp = (historical_data["close"] * historical_data["volume"]).rolling(window=12).sum() / historical_data[
        "volume"
    ].rolling(window=12).sum()
...
def fractal_adaptive_moving_average_strategy(
    ...
    def calculate_frama(data):
        ...
        def calc_fractal_dimension(high, low, period):
            n1 = (
                (np.log(high.rolling(period).max() - low.rolling(period).min()) - np.log(high - low))
                .rolling(period)
                .mean()
            )
...
def bollinger_band_width_strategy(
    ...
    if sentiment_score <= 20 and portfolio_qty > 0:
        ...
        quantity_to_buy = min(
            int((max_investment * 1.5) // current_price), int(account_cash // current_price)
        )  # Strong buy
        if quantity_to_buy > 0:
            return ("strong buy", quantity_to_buy, ticker)
...
def ichimoku_cloud_strategy(ticker, current_price, historical_data, account_cash, portfolio_qty, total_portfolio_value):
    ...
    conv_line = (
        historical_data["high"].rolling(window=window1).max() + historical_data["low"].rolling(window=window1).min()
    ) / 2
    base_line = (
        historical_data["high"].rolling(window=window2).max() + historical_data["low"].rolling(window=window2).min()
    ) / 2
    leading_span_a = (conv_line + base_line) / 2
    leading_span_b = (
        historical_data["high"].rolling(window=window3).max() + historical_data["low"].rolling(window=window3).min()
    ) / 2
